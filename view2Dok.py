...
try:
    ...
    first_line = data[0][1:].strip().split("\t") 
    ...
    for line in data[1:]:
        ...
        parts = line.split("\t")
        pos = list(map(float, parts[0][1:-2].split(",")))  # por corchetes 
        vel = list(map(float, parts[1][2:-2].split(",")))  
        positions.append(pos)
        velocities.append(vel)

    # Conversión a numpy arrays
    positions = np.array(positions).reshape(frames, particles, 2)  
    velocities = np.array(velocities).reshape(frames, particles, 2)  
    ...
    # Calculo de la distribución teórica de Maxwell-Boltzmann
    k_B = 1.38e-23
    m_H = 1.67e-27
